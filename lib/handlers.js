 ...
 */
 ...
 handlers.index = (data,callback)=>{
  if(data.method =='get'){
    ...
    let templateData = {
      'head.title' : 'Uptime Monitoring, made it simple',
 ...
 handlers.accountCreate = (data,callback)=>{
  if(data.method =='get'){
    ...
    let templateData = {
      ...
      'head.description' : 'Signup is easy and take only a few seconds',
      'body.class' : 'acountCreate'
    };
    //Read a template as a string

    helpers.getTemplate('acountCreate',templateData,(error,str)=>{
      if(!error && str){
        //Add the universal header and footer
        helpers.addUniversalTemplates(str,templateData,(err,str)=>{
          if(!err && str){
 ...
 handlers.sesionCreate= (data,callback)=>{
  if(data.method =='get'){

    //Prepare data for interpolation
    let templateData = {
      'head.title' : 'Login to your Accountt',
...
handlers.checksEdit = (data,callback)=>{
 ...
 handlers.public = (data,callback)=>{
  if(data.method == 'get'){
   // Get fileName get Requested
   let trimmedAssetName = data.trimmedPath.replace('public','').trim();
     if(trimmedAssetName.length >0){
      helpers.getStaticAsset(trimmedAssetName,(err,data)=>{
        if(!err && data){
            // Determine Content-Type, default plain text
...
handlers._checks.post = function(data,callback){
  ...
  //console.log(typeof(data.payload.successCodes));
